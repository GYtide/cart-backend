# -*- coding: utf-8 -*-
"""
Created on Thu Mar 16 21:34:22 2023
...
class Rimg1Dsyn():
    ...
    def plot_img1D(self ,minv=0, maxv=255, newcmp ='gray',figsize=(10,4)):
            ...
            trange1 =  self.datet0 + timedelta(milliseconds= int( self.ttg[0,0] *1000) )
            trange2 =  self.datet0 + timedelta(milliseconds= int( self.ttg[0,-1]  * 1000) )
            ...
            ax1.set_title("CART Radio 1D Image " +'{:3.0f}MHz '.format(self.freq) + self.polss + ' '+ self.dateobs)
...
class RspecSyn():
    ...
    def plot_specd(self ,minv=0, maxv=255, newcmp ='jet',figsize=(10,4)):
            ...
            ax1.set_title("CART Radio Dynamic Spectrum "  + self.polss + ' '+ self.dateobs)
            ax1.set_xlim([trange1,trange2])
            ax1.set_ylim([self.yyg[0,0],self.yyg[-1,0] ])
            #ax.xticks(rotation=70)
            ax1.xaxis.set_major_formatter( mdates.DateFormatter('%H:%M:%S'))
...
def gen1Dimg( fnspec, file1, *fnimg ,start_times='2022-01-01T00:00:00', end_times='2022-01-01T00:05:00', time_resolved=None):
    '''
    ...
    figspec = rspec.plot_specd()
    
    ###################################
    rimg = Rimg1Dsyn( file1 )
    rimg.readdata()
    rimg.getdatat( t1, t2)
    fig1 = rimg.plot_img1D()
    figimg = [fig1]
    if len(fnimg) > 0 :
        for ii in range( len(fnimg)):
          ...
          figimg.append(  rimg2.plot_img1D() )
...
if __name__ == '__main__' :
    ...
    fn = 'D:/sswdb/DSRT数据/概图数据/'  +   'CARTImag1DCheck_151MHz_II_NS_20220417.fits'
    fn2 = 'D:/sswdb/DSRT数据/概图数据/'  +   'CARTImag1DCheck_151MHz_II_EW_20220417.fits'
    
    fnspec = 'D:/sswdb/DSRT数据/概图数据/'  +   'CARTSpecCheck_II_20220417.fits'
  
   
    figspec,figimg = gen1Dimg( fnspec,fn, fn2, start_times='2022-04-17T03:25:00', end_times='2022-04-17T03:45:00' )
    ### 注意 figimg 是一个列表 fig 对象的
